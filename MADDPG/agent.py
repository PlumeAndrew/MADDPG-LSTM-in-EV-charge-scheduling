...
from networks_maddpg import ActorNetwork, CriticNetwork
...
class Agent:
    ...
    def __init__(self,actor_dims,critic_dims,n_actions,n_agents,agent_idx,chkpt_dir,
                 alpha=args.lr_a,beta=args.lr_c,fc1=args.fc_1,fc2=args.fc_2,gamma=args.gamma,tau=args.tau,
    ):
        self.gamma = gamma
        self.tau = tau
    ...
    def choose_action(self, observation, noise_en=True):
        # state = T.tensor([observation], dtype=T.float).to(self.actor.device)
        state = T.tensor([observation], dtype=T.double).to(self.actor.device)
